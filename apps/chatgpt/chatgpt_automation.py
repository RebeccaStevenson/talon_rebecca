"""ChatGPT browser automation and cross-platform shortcuts."""
...
from talon import Module, actions, app, clip, ui
...
def _chatgpt_modifier(platform: str | None = None) -> str:
    return "cmd" if (platform or app.platform) == "mac" else "ctrl"


def _chatgpt_shortcut(suffix: str, platform: str | None = None) -> str:
    separator = "" if suffix.startswith(":") else "-"
    return f"{_chatgpt_modifier(platform)}{separator}{suffix}"


# TODO: Make these conditional on ChatGPT being active?
@module.action_class
class Actions:
    def chatgpt_mod(suffix: str):
        """Press a ChatGPT shortcut with the platform-specific modifier."""
        actions.key(_chatgpt_shortcut(suffix))
